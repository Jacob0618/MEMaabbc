...
def generate():
    """在测试集上按 batch 进行生成，并返回所有样本的预测 token id。

    流程：
    1) 切换到评估模式并关闭梯度；
    2) 从测试迭代器读取一批输入并搬到 device；
    3) 使用 beam search 生成每个样本的结果；
    4) 将每批结果累积到列表，直到遍历完整个测试集。
    """
    # 推理模式：关闭 dropout 等训练行为
    model.eval()
    # 保存全量预测结果（二维 list：每个元素是一条生成序列的 token id）
    idss_predict = []
    with torch.no_grad():
        while True:
            # 读取一批测试数据（最后一个返回值此处不使用）
            task, source, source_mask, whole_word, _ = exp_iterator.next_batch_test()
            # 将输入张量移动到当前计算设备（CPU/GPU）
            task = task.to(device)  # (batch_size,)
            source = source.to(device)  # (batch_size, seq_len)
            source_mask = source_mask.to(device)
            whole_word = whole_word.to(device)

            # 通过分组 beam search 生成文本（每个样本返回 1 条候选）
